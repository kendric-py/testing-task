...
class ParserXmlRatesBank:
    ...
    def get_currency_rate(cls, currency_id: str) -> dict | None:
        """Основной метод запуска поиска валюты."""
        rates_bank = cls.__get_currency_rates()
        cls.__write_new_rates(rates_bank=rates_bank)
        print(cls.__find_target_currency(currency_id))


ParserXmlRatesBank().get_currency_rate('R01135')